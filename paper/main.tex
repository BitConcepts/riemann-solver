...
\emph{Step 1.} Since $C(u) = O(e^{2u})$ (ratio of polynomial-exponential terms) and the dominant $n=2$ term of $\varepsilon^*$ decays as $e^{-6\pi e^{2u}}$, direct logarithmic differentiation gives $\frac{d}{du}\log\lambda \leq 2 - 6\pi e^{2u}$ (see \texttt{docs/uniform\_wtail\_bound.md}).
...
\[
|W_{\mathrm{tail}}(u)| \leq C\,\varepsilon^*(u), \quad \varepsilon^*(u) := 2\sum_{n=2}^{\infty} n^4\,e^{-\pi(n^2-1)e^{2u}},
\]
...
for all $u \in [1, 3]$, while the certified minimum margin is $91.3$.  Hence
$(\log\Phi)'' = W_1 + W_{\mathrm{tail}} \leq -93.1 + 1.82\times 10^{-25} < 0$.
See \texttt{proof/verify\_ia\_1\_to\_3.py} (SHA256: \texttt{1BB9E9DECF13580C...}).
\end{proof}
...
\end{align*}
Hence $W_1(3) + 204\,\varepsilon^*(3) < -1000 + 204\cdot 10^{-1636} < 0$.
...
\textbf{Corrected tail values $\varepsilon^*(u) = 2\varepsilon_{\mathrm{old}}(u)$:} $\varepsilon^*(1.0) \approx 1.82\times 10^{-29}$,
$\varepsilon^*(1.5) \approx 1.96\times 10^{-81}$, $\varepsilon^*(2.0) \approx 1.07\times 10^{-222}$,
$\varepsilon^*(3.0) < 10^{-1636}$. See \texttt{proof/verify\_algebraic\_core.py}.
\end{proof}
...
$(\log\Phi)''(u) < 0$ for $u \geq 3$ & Explicit bound & monotonicity+$\varepsilon^*(3)<10^{-1636}$ & PROVED \\
...
\begin{tabular}{@{}lll@{}}
\toprule
Cert & Output file & SHA256 prefix (16 chars); full hash in \texttt{docs/certificate\_hash\_table.md} \\
\midrule
...
\end{tabular}
\end{center}
Prefixes shown for readability. Full 64-character SHA256 hashes are in \texttt{docs/certificate\_hash\_table.md}. To verify: run the corresponding script and compare.
...
\item[C-17.] \textbf{Tail prefactor bound corrected from $n^4$ to $2n^4$.}
  Status: $\checkmark$ RESOLVED. $B_n(u)/n^4 \leq 1+3/(2\pi-3) < 2$;
  corrected script recertifies $[1,3]$, minimum margin~91.3.

\item[C-18.] \textbf{Script renamed to \texttt{verify\_ia\_1\_to\_3.py}.}
  Status: $\checkmark$ DONE.
\end{enumerate}
