...
\begin{remark}
The conditions of Theorem~\ref{thm:polya} are not individually sufficient. The function $e^{-|t|^p}$ satisfies~(i)--(iii) for all~$p \geq 2$, and its cosine transform has only real zeros when~$p$ is an \emph{even integer}~\cite[\S2, Example~2.1]{csordas1989}. For $p = 3$, the function $e^{-t^3}$ (defined on~$[0,\infty)$) is not even, and the even extension $e^{-|t|^3}$ satisfies~(i)--(iii) but its decay $e^{-|t|^3}$ fails~(iv) since $3 < 2 + \delta$ requires $\delta \leq 1$ and the theorem needs the decay to dominate \emph{any} polynomial times Gaussian. Our argument-principle computation finds 4~complex zeros of $\int_0^\infty e^{-t^3}\cos(zt)\,dt$ in $[5,15] \times [-5,5]$ (winding number~6 vs.\ 2~real zeros), confirming that the conditions are not vacuous.

Our kernel~$\Phi$ decays as $e^{-\pi e^{2u}}$, which satisfies condition~(iv) with any~$\delta > 0$ and margin exceeding $10^{12\,000\,000}$ at~$u = 8$.
\end{remark}
