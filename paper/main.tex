...
All computational results are reproducible via publicly available Python scripts at \repourl. A Lean~4 formalization (\texttt{lean4/RHProof/Basic.lean}) has been machine-checked via \texttt{lake build} on leanprover/lean4:v4.16.0 with Mathlib (commit \texttt{a6276f4c}); the build of 1{,}976 modules completes with zero errors and zero \texttt{sorry} declarations. Six algebraic theorems are proved from Mathlib: \texttt{h\_pos\_for\_nonneg}, \texttt{log\_h\_d2\_neg}, \texttt{log\_phi1\_d2\_neg}, \texttt{phi\_positive\_exp}, and the newly added \texttt{phi1\_pos} ($\varphi_1(u) > 0$ for $u \geq 0$) and \texttt{phi1\_decay\_bound} ($\varphi_1(u) \leq 2\pi e^{2u} e^{-\pi e^{2u}}$ for $u \geq 0$). The main theorem \texttt{riemann\_hypothesis} depends on 13 project-specific axioms and the 3 universal Lean kernel axioms (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}).
...
We subjected every link in the proof chain to 36~systematic falsification attacks organized in 7~batches. Each attack attempts to \emph{break} the proof by finding a counterexample or inconsistency. All 36~attacks failed. A representative selection:
...
\textbf{(ii) Closes the de Bruijn--Newman gap.} Rodgers and Tao~\cite{rodgers2020} prove $\Lambda \geq 0$; Platt and Trudgian~\\cite{platttrudgian2021} establish $\Lambda \leq 0.2$ (improving on the Polymath~15 bound of $0.22$). Our result implies $\Lambda \leq 0$: if $\Xi = H_0$ has only real zeros, then by de~Bruijn's theorem the zeros remain real for all $t > 0$, so~$\Lambda = \inf\{t : H_t \text{ has only real zeros}\} \leq 0$. Together: $\Lambda = 0$. This closes the interval $[0, 0.22]$ to the single point~$\{0\}$.

\textbf{(iii) Bypasses the Connes det$_\mathrm{reg}$ convergence gap.} Connes~\cite{connes2026} identifies an open gap: the regularised determinant $\det_\mathrm{reg}(D^{(\lambda,N)}_{\log} - z)$ is conjectured to converge to~$\Xi(z)$, but this convergence is not proven (CCM 2025,~\S8; independently confirmed open by the axiom count of~\cite{sliwinski2026}). Sliwinski~\cite{sliwinski2026} proves the error satisfies $\varepsilon(\lambda,N) \geq 1/(4\ln\lambda)$, bounding the convergence rate from below. Our approach uses the Fourier cosine representation $\Xi(t) = \int_0^\infty \Phi(u)\cos(tu)\,du$ directly; no spectral convergence is required. As a consistency check, we reproduce the CvS eigenvalue plateau using the reference implementation~\cite{groskin2026}: at $N = 10$ basis functions, $\log_{10}|\lambda_{\min}(c)|$ reaches $-34.2$ at $c = 53$ with $\Delta = 0.04$ between the last two cutoffs (essentially flat), confirming the plateau predicted by the stabilisation theorem of Connes--van~Suijlekom. Ground-state eigenvectors exhibit $c$-invariance (overlap $\geq 0.9999$, confirmed at $N = 3$ and $N = 5$). We do not rely on these spectral results.
